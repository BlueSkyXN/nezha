...
update_script() {
    ...
    curl -sL https://${GITHUB_RAW_URL}/script/install_cn.sh -o /tmp/nezha.sh
    new_version=$(grep "NZ_VERSION" /tmp/nezha.sh | head -n 1 | awk -F "=" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
    if [ ! -n "$new_version" ]; then
        echo "脚本获取失败，请检查本机能否链接 https://${GITHUB_RAW_URL}/script/install_cn.sh"
        return 1
    fi
...
show_menu() {
    printf "
    ${green}哪吒监控管理脚本${plain} ${red}${NZ_VERSION}${plain} ${yellow}[CN]${plain}
